...
void Moto_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6; //用于对两个驱动模块的使能（ENA/ENB）
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
...
void turn_Left(){
	...
	delay_ms(50);			//留出时间等待HMC5883L获得数据进行比较
	stop(); //test---停止
	delay_ms(2000);			//留出时间等待HMC5883L获得数据进行比较
}
//转向控制右转试探函数
void turn_Right(){
